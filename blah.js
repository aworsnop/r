document.querySelector("#tltbt > div.Nl").remove();
document.querySelector("#tl_ > div.wf > div > form > div.Zj > input").setAttribute('hidden', 'hidden');
document.querySelector("#tl_ > div.wf > div > form > div.bb.Vj.Xj.Wj").setAttribute('hidden', 'hidden');
document.querySelector("#tl_ > div.wf > div > form.bg").style.height = "0px";
document.querySelector("#tltbt > div.wm > div > div.mf").textContent=document.querySelector("#tl_ > div.wf > div > form > div.Zj > input.Pj.ak").value




/* document.querySelector("#tltbt > div.wm > div > div").setAttribute('hidden', 'hidden');

document.querySelector("#tl_ > div.wf > div > form").remove();
 document.querySelector("#tltbt > div.Nl > div.Ll.d.xq.ef.sm.um").remove();document.querySelector("#tl_ > div.wf > div > form > div.Zj > input").setAttribute('hidden', 'hidden');

document.querySelector("#tl_ > div.wf > div > form > div.bb.Vj.Xj.Wj").setAttribute('hidden', 'hidden')

document.querySelector("#tl_ > /*div.wf > div > form > div.Zj").remove();document.querySelector("#tl_ > div.wf > div > form").remove();*/


